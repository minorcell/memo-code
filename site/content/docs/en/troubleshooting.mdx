...
## `search_files` Returns Unexpected Results

Symptom:

- `search_files` returns no matches or too many matches

Fix:

- use a glob pattern relative to the input `path` (for example `**/*.ts`)
- use `excludePatterns` to reduce noise (for example `node_modules/**`)
