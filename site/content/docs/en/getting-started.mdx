...
## One-shot and Resume Flags

- One-shot run: `memo --once "your prompt"` (or `memo -once "your prompt"`).
- Continue latest local session: `memo --prev` (or `memo -prev`).
...
Start local web server + UI:

```bash
memo web --host 127.0.0.1 --port 5494 --open
```

- npm distribution already includes web assets.
- If running from source checkout, run `pnpm run build` first.
- Login credentials come from `~/.memo/server.yaml` (`auth.username` / `auth.password`).
- The UI includes chat timeline, project/session management, MCP management, and skills management.
