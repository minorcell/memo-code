...
description: 'Built-in tools, categories, and runtime controls.'
order: 4
...
- filesystem tools are constrained to allowed roots (runtime `cwd` + `MEMO_FS_ALLOWED_ROOTS`).
- `read_media_file` returns JSON text with `type`, `mimeType`, and base64 `data`.
