...
- `supports_verbosity`
    - Controls whether verbosity-related request options are enabled.
- `context_window`
...
### About `max_prompt_tokens`

`max_prompt_tokens` is retained for backward compatibility in config files.

- It is no longer the primary control for session context size.
- Prefer `model_profiles.*.context_window` for new configuration.
