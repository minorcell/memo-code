...
- 根目录脚本由 `package.json` 驱动，需 Bun ≥1.1，推荐安装 `rg` 加快搜索；类型与路径别名见 `tsconfig.json`。
- 运行时配置与日志默认位于 `~/.memo/`，可通过环境变量 `MEMO_HOME` 重定向。

## 构建、测试与开发

- 安装依赖：`bun install`。
- 本地运行（自动选择 TUI/单轮模式）：`bun start` 或 `bun start "prompt" --once`。
- 生成可分发包：`bun run build`；生成可执行二进制：`bun run build:binary`（产出 `memo`）。
- 格式化：`bun run format`（写入）/ `bun run format:check`（CI 只检查）。
- 测试：`bun test` 全量；或按包：`bun run test:core`、`bun run test:tools`、`bun run test:cli`。CI 使用 `bun run ci` 串联格式检查、核心/工具测试与构建。
- 本地常见故障：未设置 `OPENAI_API_KEY`/`DEEPSEEK_API_KEY` 会提示交互输入；非 TTY 环境会自动退回单轮模式。
- 开发迭代可用 `bun test --watch path/to/file.test.ts` 缩短反馈时间。

## 代码风格与命名

- 语言：TypeScript + ESM；保持 Core（逻辑）、Tools（能力）、CLI（UI/接线）边界清晰。
- 使用 Prettier 统一格式，2 空格缩进；遵循 `bun run format` 输出，不手动改规则。
...
- 定位执行：`bun test path/to/file.test.ts`；新增功能需覆盖错误分支与配置边界。
...
- PR 需包含：变更摘要、关联 issue（如有）、风险/回滚提示、验证步骤（如 `bun test`, `bun run format:check`）；仅 UI 输出变更时附 TUI 截图。
